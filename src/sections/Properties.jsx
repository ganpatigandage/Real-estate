    ...
    <div className={`${darkMode ? 'dark: bg-black' : 'light bg-transparent'}`}>
      <section id='properties' className='lg:w-[90%] m-auto lg:px-20 px-6 py-20 w-full flex flex-col justify-center items-start gap-10 '>
        ...
        <div className='flex flex-col justify-center items-start gap-4'>
          <h1 data-aos="zoom-in" className='text-red-500 text-[30px] dark:text-white'>PROPERTIES</h1>
          <h1 data-aos="zoom-in" className='text-black text-[40px] font-semibold leading-10 dark:text-white'>Explore the Latest</h1>
        </div>

        {/* properties grid statrs from here */}

        <div id='grid-box' className='w-full grid lg:grid-cols-3 grid-cols-1 justify-center item-center gap-8'>
          {
              ...
              <div data-aos="zoom-in" data-aos-delay="200" key={index} className='bg-white dark:bg-gray-800 rounded-xl w-full'>
                ...
                <div className='px-6 py-3 flex flex-col justify-center items-start gap-2 w-full'>
                  ...
                  <div id='owner-info' className='flex justify-between items-center w-full mt-2'>
                    ...
                    <div className='flex justify-center items-center gap-4'>
                      <div className='p-2 border-gray-200 hover:bg-black cursor-pointer transform hover: scale-110 transition-transform duration-300'> <FaShareAlt className='size-4 text-red-400' /></div>
                      <div className='p-2 border-gray-200 hover:bg-black cursor-pointer transform hover: scale-110 transition-transform duration-300'> <FaHeart className='size-4 text-red-400' /></div>
                      <div className='p-2 border-gray-200 hover:bg-black cursor-pointer transform hover: scale-110 transition-transform duration-300'> <FaPlus className='size-4 text-red-400' /></div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="flex justify-center w-full">
          <button data-aos="zoom-in" data-aos-delay="400" class="bg-red-600 text-md m-auto px-10 py-4 text-white font-semibold rounded-xl hover:bg-black dark:hover:bg-red-700 cursor-pointer aos-init aos-animate">LOAD MORE</button>
        </div>
      </section>
    </div>
  )
